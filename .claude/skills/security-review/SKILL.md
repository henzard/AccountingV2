...
# /security-review — Red Team / Blue Team Source Review
...
- **RT4 — Secrets, crypto & supply chain:** hardcoded/committed secrets,
  secrets in the client bundle or git history, weak hashing/randomness,
  homegrown crypto, insecure cookie flags, verbose errors leaking internals,
  vulnerable/unpinned deps, risky postinstall scripts.
...
```text
ID & Title
Severity: Critical | High | Medium | Low   (CVSS-style reasoning)
Location: file:line
Category: OWASP Top 10 2021 / CWE id
...
`deep-review` pipeline. (If you ran `bootstrap-labels.sh` the labels already
...
Dependabot, Semgrep/CodeQL, gitleaks, and OWASP ZAP for coverage LLMs miss.
...
- This skill is portable — copy `.claude/skills/security-review/` into any repo;
