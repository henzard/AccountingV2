...
# /qa-lead — the Expert QA Lead who owns the outcome
...
This skill composes the rest of the kit: it uses **`quality-review`** for the specialist
roster, **`security-review`** when the surface warrants it, **`verification-quality`** as
the completion gate and the anti-pattern catalogue, **`branch-hygiene`** for never-losing
work, and the **completeness-loop** discipline from **`deep-review`**. What `qa-lead`
...
- **Manager posture:** decompose → dispatch → monitor → fix → re-verify. Commit + push
  after every cycle (`branch-hygiene`) so a swept context never loses work.

---

## Phase 0 — Take ownership & set up (never lose work)

1. Follow **`branch-hygiene`**: confirm the default branch, `git status` clean, then branch
   `git checkout -b qa/owned-quality-$(date +%Y%m%d)` off the default. Work here; commit +
   push every cycle.
2. Discover this repo's **truth commands** per **`verification-quality` Step 0**: the real
...
- **Enumerate the user journeys:** trace the _critical paths_ a real user takes end-to-end
  ...
  spine — most value, most risk. ([E2E best practices][e2e], [test pyramid][pyr])
...
- Run the **real** suite with coverage and capture per-module numbers
  (`verification-quality` / `quality-review`): e.g. `npx vitest run --coverage`,
...
- Run the static/type/lint gate. Record the **current red** so you can tell _new_
  breakage from pre-existing (anti-pattern #4 in `verification-quality`).
...
**`quality-review`** (Test authenticity & the six-layer matrix; UX/Nielsen+WCAG; UI; API;
Backend SOLID; Database; and the extended **sync / orchestration / remaining-backend**
agents _only when that subsystem exists_). On top of that roster, `qa-lead` runs these
...
- **Exploratory / journey agent** — actually _drive each critical journey end-to-end_ (run
  the app per `verification-quality` Step 2; for a UI, click through it; for an API, hit the
  endpoints; for a CLI, run the commands). Assert the **real outcome** the charter names —
  not "page loaded". Probe where a curious user would. ([autonomous exploratory testing][aet])
...
- **Integration / contract agent** — cross-component and external-boundary behavior: does
  ...
  (`verification-quality` anti-pattern #2). For data mutations, target the **integration-edge**
...
- **Security agent** (when the surface warrants) — invoke **`security-review`** (injection,
...
3. **Ship a regression test that exercises the real failing vector** and now passes
   (`verification-quality`: "every fix ships with a regression test").
4. **Re-run** the focused test, then the touched component's suite. Commit + push with a
   ...
   File anything you _don't_ fix this cycle as a GitHub issue (reuse `quality-review`'s label
...
Each cycle, re-aim with a **completeness-critic** question (from `deep-review` Phase 10):
...
Close with what you did, in evidence (mirror `verification-quality`'s "what I ran and what I
...
[aet]: https://testquality.com/autonomous-exploratory-testing-ai-agents/ "AI Autonomous Exploratory Testing"
