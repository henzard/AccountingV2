...
describe('notificationStore', () => {
  describe('store defaults (no seeding)', () => {
    it('has default preferences', () => {
      const { preferences } = useNotificationStore.getState();
      expect(preferences).toBeDefined();
      expect(preferences.eveningLogPromptEnabled).toBe(true);
      expect(preferences.eveningLogPromptHour).toBe(19);
      expect(preferences.meterReadingReminderEnabled).toBe(true);
    });
