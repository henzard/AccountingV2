...
export class UnitRateCalculator {
  calculate(
    current: MeterReadingEntity,
    previous: MeterReadingEntity,
  ): Result<RateCalculationResult> {
    if (current.meterType !== previous.meterType) {
      return createFailure({
        code: 'TYPE_MISMATCH',
        message: 'Both readings must be the same meter type',
      });
    }
    ...
    if (consumptionUnits <= 0) {
      return createFailure({
        ...
        message: 'Current reading must exceed the previous reading',
      });
    }
