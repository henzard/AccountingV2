...
Lines that take money OFF the slip — a discount, voucher, coupon, loyalty or promotional saving, a returned/refunded item, or a deposit refund — are real line items: emit each one with a NEGATIVE amount_cents (e.g. "DISCOUNT -5,00" → amount_cents: -500) and give it the same suggested_envelope_id as the item it applies to when that is clear. Never fold a discount into another item's amount and never emit it as a positive amount. The line items, signed, should add up to the slip total. Do not emit VAT, subtotal, tender, change, or rounding lines as items.
