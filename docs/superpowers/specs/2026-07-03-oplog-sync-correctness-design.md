...
### Slice-5 follow-ups (found in Task-1 review, 2026-07-04)

- **Debt payment `is_paid_off` divergence (Important, tracked):** `LogDebtPaymentUseCase` emits 3 oplog ops per payment (balance decrement + total_paid increment as `increment` ops — forced because the server `increment` RPC is single-field — plus an `is_paid_off` `update` op). The batch is per-op non-atomic server-side, so a transient rejection of the balance op while the client-computed `is_paid_off` op applies could show "paid off" with a nonzero remote balance (no money lost — balance is atomic per its own op; only the derived flag goes stale until DLQ retry). Proper fix: derive `is_paid_off` server-side (generated column / trigger off `outstanding_balance_cents`) so it can't be independently popped — do this when the remote schema is next touched (slice 6 / remote deploy), or have the SyncEngine retry a payment's op-set as a bundle.
- **`ReconcileEmergencyFundTypeUseCase` deletion (Minor):** spec §8 lists it for deletion (persistent EMF scope obsoletes it); slice-5 Task 1 migrated it onto the oplog rather than deleting. Delete it in slice 6 cleanup (do not leave it migrated-but-dead).
